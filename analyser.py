...
from common.dataloader import get_cifar10_dataset, get_mnist_dataloader
from common.cli import get_parser
from common.net import VGG, MNIST_Net
...
INPUT_SHAPE = [BATCHSIZE, 3 if USE_VGG else 1, 32, 32]
DEVICE      = 'cuda'
...
def load_calibration_dataset() -> Iterable:
    ...
    # ------------------------------------------------------------
    if USE_VGG:
        _, ds_valid = get_cifar10_dataset(args)
    else:
        _, ds_valid = get_mnist_dataloader(args)
    dataset = [data[0] for data in ds_valid]
    return dataset
...
if USE_VGG:
    model = VGG()
    model.load_state_dict(torch.load(r"./ckpt/vgg.cifar.pretrained.pth")["state_dict"])
    model = model.to(DEVICE)
else:
    model = MNIST_Net()
    model.load_state_dict(torch.load(r"./ckpt/best.pth"))
    model = model.to(DEVICE)
